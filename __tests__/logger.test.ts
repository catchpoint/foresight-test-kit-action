...
describe("Logger", () => {
    ...
    it('tests for logger debug', async () => {
        ...
        expect(mockCoreDebug).toHaveBeenCalledWith(LOG_HEADER + ' ' + "test_debug");
    ...
    it('tests for logger info', async () => {
        ...
        expect(mockCoreInfo).toHaveBeenCalledWith(LOG_HEADER + ' ' + "test_info");
        mockCoreInfo.mockRestore();
    });

    it('tests for logger warning', async () => {
        const mockCoreWarning = jest.spyOn(core, 'warning')
        .mockImplementation((data: any) => { 
            return data;
        });
        logger.warning("test_warning");
        expect(mockCoreWarning).toHaveBeenCalledWith(LOG_HEADER + ' ' + "test_warning");
        mockCoreWarning.mockRestore();
    });


    it('tests for logger notice', async () => {
        const mockCoreNotice = jest.spyOn(core, 'notice')
        .mockImplementation((data: any) => { 
            return data;
        });
        logger.notice("test_notice");
        expect(mockCoreNotice).toHaveBeenCalledWith(LOG_HEADER + ' ' + "test_notice");
    ...
    it('tests for logger error with string', async () => {
        ...
        expect(mockCoreError).toHaveBeenCalledWith(LOG_HEADER + ' ' + "test_error");
