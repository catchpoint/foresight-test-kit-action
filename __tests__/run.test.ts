...
describe("Generate Cli Commands", () => {
    ...
    it('tests for generating upload test command', async () => {
        const command = await  run.generateCliCommand("api_key", FRAMEWORK_TYPES.TEST, "pytest", ["./reports", "./target"]);
        expect(command).toEqual(`thundra-foresight-cli upload-${FRAMEWORK_TYPES.TEST} -a api_key -f PYTEST -ud ./reports -ud ./target`);
    });

    it('tests for generating upload coverage command', async () => {
        const command = await  run.generateCliCommand("api_key", FRAMEWORK_TYPES.COVERAGE, "jacoco", ["./reports", "./targets/**"]);
        expect(command).toEqual(`thundra-foresight-cli upload-${FRAMEWORK_TYPES.COVERAGE} -a api_key -f JACOCO -ud ./reports -ud ./targets/**`);
    });
