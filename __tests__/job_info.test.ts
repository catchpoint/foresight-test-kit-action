...
describe("JOB INFO", () => {
    ...
    it('tests job info', async () => {
        const octokit = new Octokit("test_github_token");
        const jobInfo = require('../src/actions/job_info');
        const fs = require('fs');
        let rawdata = fs.readFileSync(path.join(__dirname, '/data/job_info_result.json'));
        let result = JSON.parse(rawdata);
        const mockListWorkflow = jest.spyOn(octokit.rest.actions, 'listJobsForWorkflowRun')
